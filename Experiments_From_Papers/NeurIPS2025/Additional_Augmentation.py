...
def train_and_get_curves(lambda_field, device, train_loader, test_loader,
    ...
    for _ in range(5):
        for imgs, lbls in train_loader:
            ...
            if augment_angles is not None:
                imgs = torch.cat([TF.rotate(imgs, a, fill=(fill_val,)) for a in augment_angles], dim=0)
                lbl_bin = lbl_bin.repeat(len(augment_angles),1)
...
def main():
    ...
    p7_aug,  p8_aug  = train_and_get_curves(0.0,device,train_loader,test_loader,
                                            kernel_x,kernel_y,Vx8,Vy8,angles,fill_val,
                                            augment_angles=[0,45,315])
    p7_hyb,  p8_hyb  = train_and_get_curves(1.0,device,train_loader,test_loader,
                                            kernel_x,kernel_y,Vx8,Vy8,angles,fill_val,
                                            augment_angles=[0,45,315])
    ...
    plt.plot(angles,p7_aug,':o', label='aug [0,45,315]')
    plt.plot(angles,p7_hyb,'-.o',label='aug+[λ=1]')
    plt.xlabel('Rotation Angle (°)')
    plt.ylabel('P(model predicts 7)')
    plt.title("Digit '7': comparison")
    ...
    plt.plot(angles,p8_aug,':o', label='aug [0,45,315]')
    plt.plot(angles,p8_hyb,'-.o',label='aug+[λ=1]')
    plt.xlabel('Rotation Angle (°)')
    plt.ylabel('P(model predicts 8)')
    plt.title("Digit '8': comparison")
    plt.ylim(0,1); plt.legend(); plt.grid(True); plt.tight_layout(); plt.show()
