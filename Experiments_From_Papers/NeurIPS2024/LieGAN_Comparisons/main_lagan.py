...
if __name__ == '__main__':
    ...
    if args.task == 'gaussian':
        ...
        mean = [1,1,1]
        cov = np.array([[4,0,0],[0,1,0],[0,0,0.0001]])
        X = np.random.multivariate_normal(mean, cov, size=args.dataset_size)
        y = [(X[i][0]-mean[0])**2+4*(X[i][1]-mean[1])**2 for i in range(X.shape[0])]
        dataset = torch.utils.data.TensorDataset(torch.tensor(X).float(),torch.tensor(y).float())
        n_dim = 3
        n_channel = args.n_channel
        d_input_size = 4 #is this right?
    
    elif args.task == 'surface':
        # define the dataset
        mean = [0,0]
        cov = np.array([[4,0],[0,4]])
        X = np.random.multivariate_normal(mean, cov, size=args.dataset_size)
        y = [X[i][0]**3 - X[i][1]**2 for i in range(X.shape[0])]
        dataset = torch.utils.data.TensorDataset(torch.tensor(X).float(),torch.tensor(y).float())
        n_dim = 2
        n_channel = args.n_channel
        d_input_size = 3
